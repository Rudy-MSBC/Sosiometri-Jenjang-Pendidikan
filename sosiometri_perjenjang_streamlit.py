...
st.set_page_config(page_title="Sosiometri Perjenjang PAUD–SLTA", layout="centered")
st.title("📊 Aplikasi Sosiometri & Sosiogram Multi-Jenjang (PAUD – SMA)")

jenjang = st.selectbox("Pilih Jenjang Pendidikan", ["PAUD", "SD", "SMP", "SMA"])
...
for i, q in enumerate(pertanyaan_dict[jenjang], 1):
    st.markdown(f"**{i}. {q}**")

uploaded_file = st.file_uploader("📥 Upload jawaban sosiometri dalam format Excel (kolom: Nama Siswa, Pilihan 1 ... Pilihan 18)", type=["xlsx"])
...
if uploaded_file:
    ...
    try:
        ...
        st.download_button("⬇️ Download Word", data=word_file, file_name="hasil_sosiometri.docx")
